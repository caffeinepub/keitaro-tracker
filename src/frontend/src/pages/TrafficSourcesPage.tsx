...
import { Copy, Globe, Link2, Pencil, Plus, Trash2, X } from "lucide-react";
...
// ── Postback URLs helper ───────────────────────────────────────────────────────

function getPostbackUrls(): { lead: string; sale: string } {
  const base = window.location.origin;
  const lead = `${base}/postback?click_id={click_id}&payout={payout}&status=lead`;
  const sale = `${base}/postback?click_id={click_id}&payout={payout}&status=sale`;
  return { lead, sale };
}

// ── Postback URLs Dialog ───────────────────────────────────────────────────────

interface PostbackUrlsDialogProps {
  open: boolean;
  onClose: () => void;
  tsName: string;
}

function PostbackUrlsDialog({
  open,
  onClose,
  tsName,
}: PostbackUrlsDialogProps) {
  const { lead, sale } = getPostbackUrls();
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg" data-ocid="postback.dialog">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Link2 className="w-4 h-4 text-primary" />
            Postback URLs — {tsName}
          </DialogTitle>
          <DialogDescription>
            Copy these URLs and paste them into your affiliate network as
            postback / S2S pixel URLs. The network will replace{" "}
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="text-xs font-semibold text-success uppercase tracking-wide">
              Lead event
            </Label>
            <div className="flex items-center gap-2 p-3 rounded-md bg-muted/50 border border-border">
              ...
              <Button
                ...
                onClick={() => copyUrl(lead, "Lead postback")}
                data-ocid="postback.lead.copy_button"
              >
          ...
          <div className="space-y-2">
            <Label className="text-xs font-semibold text-info uppercase tracking-wide">
              Sale event
            </Label>
            <div className="flex items-center gap-2 p-3 rounded-md bg-muted/50 border border-border">
              <code className="flex-1 text-xs font-mono text-muted-foreground break-all">
              ...
              <Button
                ...
                size="icon"
                className="h-7 w-7 shrink-0"
                onClick={() => copyUrl(sale, "Sale postback")}
                data-ocid="postback.sale.copy_button"
              >
                <Copy className="w-3.5 h-3.5" />
              </Button>
            </div>
          ...
          <div className="rounded-md bg-muted/30 border border-border px-3 py-2 text-xs text-muted-foreground space-y-1">
            ...
            <p>
              <code className="bg-muted px-1 rounded">{"{click_id}"}</code> —
              unique click identifier generated by this tracker
            </p>
            <p>
              <code className="bg-muted px-1 rounded">{"{payout}"}</code> —
              conversion payout amount from the network
            </p>
          </div>
        </div>
        ...
        <DialogFooter>
          <Button
            variant="outline"
            onClick={onClose}
            data-ocid="postback.close_button"
          >
            Close
          </Button>
        </DialogFooter>
...
interface TSFormProps {
  ...
  onCreated?: (ts: TrafficSource) => void;
}

function TSForm({ open, onClose, editingTS, onCreated }: TSFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTS) {
        ...
        const created = await createTS.mutateAsync({
          name: name.trim(),
          postbackUrl: postbackUrl.trim(),
          costModel,
          parameters,
...
export default function TrafficSourcesPage() {
  ...
  const [postbackFor, setPostbackFor] = useState<TrafficSource | undefined>();
  ...
  const handleCreated = (ts: TrafficSource) => {
    setPostbackFor(ts);
  };
    ...
    <div className="p-6 space-y-5">
      ...
      {formOpen && (
        <TSForm
          ...
          onClose={closeForm}
          editingTS={editing}
          onCreated={!editing ? handleCreated : undefined}
        />
      )}
      ...
      {postbackFor && (
        <PostbackUrlsDialog
          open={!!postbackFor}
          onClose={() => setPostbackFor(undefined)}
          tsName={postbackFor.name}
        />
      )}
