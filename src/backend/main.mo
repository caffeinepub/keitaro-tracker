...
actor {
  ...
  // Traffic Sources - Admin only for modifications, users can view
  ...
  // Offers - Admin only for modifications, users can view
  ...
  // Campaigns - Admin only for modifications, users can view
  ...
  // Flows - Admin only for modifications, users can view
  ...
  // Click Tracking - Public endpoint for external tracking systems (no auth required)
  public shared ({ caller }) func recordClick(campaignId : Text, ipAddress : Text, country : Text, city : Text, os : Text, browser : Text, deviceType : Text, referrerUrl : Text, landingPageUrl : Text) : async ClickEvent {
    // No authorization check - this must be callable by external tracking systems
  ...
  // Conversion Tracking - Public endpoint for external tracking systems (no auth required)
  ...
  // Statistics - Users can view
  public query ({ caller }) func getCampaignStats() : async [CampaignStats] {
    if (not AccessControl.hasPermission(accessControlState, caller, #user)) {
      Runtime.trap("Unauthorized: Only users can view campaign statistics");
    };

    [];
  };

  // Reports - Users can view
  ...
  // Domain Management - Admin only for modifications, users can view
